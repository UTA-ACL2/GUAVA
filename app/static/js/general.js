...
function run() {
	...
	for (i = 0; i <	myNodelist.length; i++) {
		...
		if(i!=0)
			params += "|@|:";
		params += myNodelist[i].getElementsByTagName('INPUT')[0].value;

	}
	document.getElementById("scriptsOrder").value = text;
	document.getElementById("scriptsParams").value = params;
...
function checkAll(){
	...
	var error = 0;
	if(document.getElementById('selected').getElementsByTagName("li").length == 0 && document.getElementById('available').getElementsByTagName("li").length > 0){
